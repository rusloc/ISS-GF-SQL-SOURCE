			...
			-- MANUAL INVOICES with tx type 'CUSTOMER'
				...
				-- join ISS DOM
						...
						--	and c."Invoice" = 'INO1-FTI-000000906'
						    ...
						    and coalesce(c."Invoice", c."Voucher") is not null
...
left join (
						select 
							...
							,c."Customer Account" 											_customer_account_cn
							,c."Company" 													_company_cn												
							,case 
								when length(c."Customer Account") <= 4
									then 'INTERCOMPANY'
								else 'EXTERNAL'
							end																_client_type
				...
				-- join total amount from SETTLEMENT operations
								...
								--				,c."Last settlement voucher" 									_to_voucher
												,c."Transaction Type" 											_type
												,c."Currency" 													_currency
												,c."Customer Account" 											_client_acc
												,c."Company" 													_company
												,case 
													when length(c."Customer Account") <= 4
														then 'INTERCOMPANY'
...
--												and c."Amount in Transaction Currency" < 0
							...
							and s._client_acc = c."Customer Account" 
